...
class AuthRepository extends BaseRepository{
    ...
    function SendOtp($credentials){
        ...
        Cache::put('otp_company' . $credentials['phone'], $otp, now()->addMinutes(1)); // Store OTP for 5 minutes, adjust 
          return [[
    ...
    function authenticate($credentials){
        ...
        $user = $this->model->where('phone', $credentials['phone'])->with('company')->first();
        $cached_otp = Cache::get('otp_company' . $credentials['phone']);
        ...
        Cache::forget('otp_company' . $credentials['phone']);  
        return [[
            'token' => $token,
            'status' => true,
            'messageff' => 'Logged in successfully.',
            'data' => $user,
        ], 'code' => 200];
    }
    ...
    public function getUserAuth(){
        $user = $this->model->where('id', auth('user')->user()->id)->with('company')->first();
        return[
                'userAuth' => $user,
