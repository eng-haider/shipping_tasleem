...
class CDCIntegrateService
{
    public function checkOrderTrExistsInCDC($tr)
    {
        ...
        try{
                ...
                $request = new Request('GET', env('CDC_API_URL'). '/api/api-external/order?uuid=' . $tr, $headers);
    ...
    public function changeCDCOrderStatus($order, $status, $tr)
    {
        ...
        try{
                ...
                $options['multipart'][0]['name'] = 'uuid';
                $options['multipart'][0]['contents'] = $tr;
      
            $options['multipart'][1]['name'] = 'status_id';
            ...
            $request = new Request('POST', env('CDC_API_URL') . '/api/api-external/order/change-status', $headers);
